...
> 版本 0.5 | 日期 2026-07-02 | 状态：讨论中（设计方向已基本收敛）
...
### 5.5 托付（异步委托）—— 你拜托 Cato 的事

> 玩家用大白话拜托 Cato 一件事（"你能帮我晚上抓点萤火虫么"），下线；下次回来 Cato 带着结果 + 一个小故事告诉你。**这是伙伴模型最动人的形态**：他趁你不在、花时间替你做了件事 = "他惦记你、为你做事"的情感金矿；天然异步（贴合网页休闲）；且守住"你控制不了他"（他会失败、带错、甚至拒绝）。**是拜托朋友，不是指挥仆人。** 与 §6.5（不在场时他过自己的日子、重逢给你看东西）天然接上。

**通用的关键：把"有界的结果"和"无界的语言/叙述"拆开。** AI 吸收无限多的"请求"和"讲法"；游戏只维护一小套可复用的"结果原语"。加内容 = 往同一套原语里倒新物品/风味，不碰机制本身（正是"可复用模式，内容慢慢倒"）。

**五步管线**（用萤火虫串一遍）：

1. **理解意图（AI）**："晚上抓萤火虫" → `{类型:带回东西, 对象:萤火虫, 条件:夜晚, 难度:中}`。
2. **映射到有界的"结果原语"（AI 分类）**——任意请求都归到其中之一：
   - **带回一个东西（fetch）** → 产出物品（萤火虫 / 花 / 贝壳 / 鱼 / 亮石头）← 萤火虫属这类
   - **发现一个地方/东西（discover）** → 产出"找到了 X" + 一段见闻 / 记忆快照
   - **尝试/练习一件事（attempt）** → 成功 / 半成 / 失败 + 故事 + 可能的小成长
   - **一段经历（vignette）** → 无实物，产出情绪 / 回忆（"替我看看日落"）
   （"去看看某人"归 discover / vignette。四类覆盖极广。）
3. **延迟概率结算（游戏，按真实时间 + 条件）——不去模拟**：等真实时间过了、条件满足（游戏内到了夜晚），**掷一个结果**（成功 / 半成 / 空手 / 意外），权重 = 难度 × Cato 心情/技能/关系 × 运气 → 结构化结果，如 `{原语:fetch, 对象:萤火虫, 结果:半成, 物品:[萤火虫×3], 细节:"一只从鼻子上飞跑了", 感受:得意}`。**不动画他做，只掷结果、让 AI 去讲——这是通用又便宜的关键。**
4. **回来时 AI 讲一个量身定做的小故事**：结构化结果 + 性格 + 记忆 → 一句鲜活的话（"我等到天黑……好多好多！抓了三只，有一只从我鼻子上飞跑啦，哈哈——给你留着呢"）。**结果数据有界，讲法无界。**
5. **落成实物 + 记忆**：萤火虫变真物品（装进玻璃罐 / 回赠你），事件存进**共同记忆**（"那晚你让我抓萤火虫"），喂给收集 + 关系。

**护栏（让它有味道、不塌）：**
- **不总成功、会带错东西**（你要萤火虫，他被青蛙吓到、叼回一块亮石头）——不可靠 = 猫味 + 守住"你控制不了他"。
- **能拒绝 / 忘了**（稀有；早期更容易放鸽子，越熟越靠谱——接上"随羁绊上升"的弧线）。
- **不可能的请求 AI 就地圆场，永不报错**（"帮我抓条龙？这岛上没有龙啦傻瓜——不过我逮到只怪虫，给你看！"）。
- **一次只接一两件**（不是任务板 / 工单系统，否则又成仆人）。
- **失败也是暖的关系节拍，甚至更暖**（"我追了一整晚也没抓到，飞太快了…对不起。不过我一直想着你呢"）。

**平台含义（又一次 Catopia 倒逼平台）**：现在的 `umicat.ai.npc` 是**当场**的 Observation→Say+Do；"托付"是一种**新形态：异步委托 / 延迟意图**（现在理解请求、过一段真实时间再结算、回来再叙述）。应沉淀成 ADR-017 之上的一层新平台原语——所有"养一个会替你做事的 AI 伙伴"的游戏都会需要。
...
- [x] **托付（异步委托）**：玩家用大白话拜托 Cato 做事、下线、回来听结果+故事（见 §5.5）。通用做法 = 有界的"结果原语" + 无界的 AI 叙述；会失败/带错/拒绝（是拜托朋友，不是指挥仆人）。**新平台原语候选**：异步委托 / 延迟意图（ADR-017 之上一层）。
