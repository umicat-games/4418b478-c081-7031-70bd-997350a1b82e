...
**At a table, `onClose` is the way out, so it is the one that has to call
`leaveTable()`.** It used to be the gear's "back to the title" button; with
that gone, closing the panel with no game behind it is getting up. A client
that stays connected while its player is on the title screen is a table on the
list of open tables with nobody at it — the fourth of the four bugs above.
