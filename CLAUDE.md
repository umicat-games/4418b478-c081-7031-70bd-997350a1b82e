...
- **A sphere, because it has no orientation.** The kit's bullet is a capsule
  ...
  anything.
- **It is the kit's own cannonball** (`td-ammo-ball`, 0.28 across, 160
  triangles), recoloured per pole — not a `SphereGeometry` built in code. The
  generated one was smooth, and everything else here is faceted: low-poly
  trees, low-poly tiles, a low-poly hero. A perfectly smooth ball among them
  reads as something imported by accident, which is most of what "the orbs
  still don't look right" was about. The kit piece shades the way the ground it
  crosses shades, because it is made the same way.

  **At the kit's size, 0.28, with nothing added.** It was scaled up once on the
  reasoning that the one object which must be read correctly every time should
  be a bigger target. Played, that is wrong in both directions — the sphere it
  replaced (0.32 with a 0.41 shell) was reported as too big, and a bullet
  taking up more of the board is a board with less room to dodge in.
  Legibility here comes from hue, which costs no area at all.

  TWO prototypes, one per pole, each with its own cloned material. Every model
  in this kit points at the same shared `colormap`, so painting one shot red
  would paint every projectile on the board red — that bug is in this
  project's history twice. A shot is a cheap clone of a prototype and touches
  no material.
...
### The colour mix SWINGS, or the swap button is optional

**A coin flip per enemy is a game with no swap button in it.** That is what
this was, and it looked completely reasonable: each crossing picked its own
colour, so the board was always about half red and half blue — which means
standing in ONE colour forever collects half of everything and is hit by
nothing. Reported as "I never need to change colour", and that is exactly
right. A choice that is never forced is not a choice.

So the mix is a tide. `redShareAt(t)` is a sine over `TIDE_PERIOD` (26s) at
`TIDE_DEPTH` 0.72, and a crossing's pole is rolled against it. For a stretch
most of what crosses is red; then it turns. Staying one colour means watching
the board fill with the thing that hurts you.

Two numbers with reasons:

- **A sine, not an alternation.** A board that flips cleanly from all-red to
  all-blue is a metronome, and the answer to a metronome is to keep time rather
  than to look. The overlap through each swing is where both colours are up and
  the decision is real.
- **Depth 0.72, not 1.** The peak of a full swing is a stretch with nothing of
  your own colour on the board — no magic coming in, nothing to do but walk —
  and a game that periodically stops paying you has a dead patch in it. At 0.72
  the thin colour is still about one crossing in seven at the extreme.

Measured at the extremes, sampled through the game's own spawn path: 49 red to
11 blue at one peak, 6 to 54 at the other.
...
because asking a question with one answer is a menu, not a choice. There is one
board now, so that case is every case.
...
- **A probe rots the same way a comment does.** Renaming the poles from
  ...
  reported as the game being broken. Grep for a renamed value across
  `playwright/` in the same commit that renames it.
...
Worth knowing: `hit-enemy.ogg` is NOT the sword's. It is in
...
**The hit's own sound is `swing-sword-sound.mp3`**, and by the numbers it is a
whoosh rather than an impact — 0.79s, peaking 86ms in, nothing in the first 40.
It was kept anyway, on the ear, once the other whoosh was gone: with nothing
underneath it, it is what a swing sounds like. `swing.ogg` and the old
`sword-hit.ogg` are both deleted — a clip that is registered and never played is
fetched and decoded on every boot for nothing, and this is a game about to be
released. Both are one `git revert` away if the question reopens.
...
Two or three words, a verb first, and **it names the thing — never "it"**. The
first set read `BUILD IT` / `UPGRADE IT` / `SELL IT`, which assumes the player
has been following along; a tutorial may not assume that. The heading is also
read in the moment BEFORE the body, so at that instant there is nothing behind
the pronoun at all.

    CHOOSE A WEAPON        Tap the weapon in the bar below
    CHOOSE WHERE TO BUILD  Stand on the marked square / Enemies come out…
    BUILD YOUR WEAPON      Press the ⟨build⟩ button on the right…
    UPGRADE YOUR WEAPON    Stand on your weapon, then press ⟨upgrade⟩…
    SELL YOUR WEAPON       Hold ⟨upgrade⟩ until it turns to ⟨sell⟩…
    SWING YOUR SWORD       Chase the enemy down and press ⟨sword⟩…
    USE THE STAFF          You have a fire staff now / Get close and press…
    AIM THE STAFF          Now stay back from this one / Hold, drag, let go

The word is **weapon** because that is what the BODY of these steps already
calls it — "tap the weapon in the bar below", "stand on your weapon". It is
worth knowing that the game uses that word for two different things: a hotbar
emplacement here, and the sword or staff in the hero's hand everywhere else.
That predates the headings and is not resolved by them.
