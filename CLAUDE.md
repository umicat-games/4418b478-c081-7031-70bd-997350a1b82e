...
**朝向能推的部分从包围盒推，推不出来的部分留成可调字段。** 「最长的那根轴
指向飞行方向」是能算的；「箭头在这根轴的哪一头」算不出来（差 180°），只能
看一眼 —— 所以它是 `HomingBolt.flip`，不是公式里的一个常数。靠看来定的那一半
要能当场翻，不用重新部署一轮。
...
`resetRun()` 既是「开一局」也是「重开一局」。升级是直接改武器对象上的字段的
（`blades.count`、`chain.jumps`……），「重开」另写一份的话，漏掉一个字段的代价
是第二局悄悄变成另一个游戏——不报错、不崩溃。共用一段之后，漏掉的那个字段
**第一局就是错的**，五秒钟就能发现。

上线当天就兑现了：`resetRun()` 在 `swarm.load()` 之前跑，而 `Swarm.clear()` →
`sync()` 缺那道「还没加载好」的门（`update()` 早就有），游戏直接启动失败。
这个洞本来只会在某个玩家按下「再来一局」时才出现。

**凡是 `load()` 之后才有的网格，每一条能碰到它的路径都要有那道门**，不只是
`update()`。

## `public/vfx/particles.png` 是黑底灰度，**只能加色混合**

那张图没有 alpha 通道，黑的地方 alpha 仍然是 1。加色混合下黑等于透明；普通
混合下黑就是黑 —— 用它做普通混合等于在草地上刷黑方片（尾迹灼烧试过，得到一条
泥巴路）。

代价是颜色会被这游戏的亮草地洗淡（约 0.35/0.78/0.45，绿通道先饱和，加什么都
偏黄白）。**身份靠形状给，不靠颜色**；`vfx.ts` 里火焰那次的结论也是这个
（「用形状而不是用不透明度」）。

贴图里的图形通常只占方片的一部分（符文约 3/4），所以**方片要比想画的东西大
一圈**，而且这个比例只能量，不能猜 —— 一个比伤害范围小 30% 的特效会让玩家
以为自己站位错了。

## 在绿底上做对比度，方向是补色

加色混合下，草地大约 (0.35, **0.78**, 0.45)：绿通道本来就高，所以加**纯红**
得到的是发黄的白，和草地只差亮度。**洋红**加在草地最弱的两个通道（红、蓝）
上，绿几乎不动 —— 色相分得开。

再进一步：把自己颜色的绿通道给到 0（`0xff0095` 而不是 `0xff2a8c`），加完绿
停在 0.78 而不是顶到 1，芯子是粉白而不是纯白，色相留得住。

## 看不清的时候，先想多塞一层，不是把数字调大

`quads()` 把整个列表做成**一个**网格 —— 多叠几层**不多一次绘制**。链式闪电从
「宽度 0.75 → 1.9」还是看不清，答案是一跳四片：宽而软的一层（这里有东西）+
窄而锐的一层压在上面（加色一叠芯子自然亮，外软内亮才像电）+ 命中星芒 +
**贴地的一圈**（贴地的东西不会被敌人挡住，是最稳的那层）。

单独一层无论调多宽，都只是一条带子。

## 瞬发动作的声音，冲击必须在开头

「显示放完了音效还在播」的根因**不是长度，是慢起**：那个 clip 前 0.4 秒几乎
无声、最响在 0.6~0.9 秒，而画面 0.4 秒就没了 —— 声音的冲击落在画面之后。

诊断方法是**量包络**（每 0.1 秒一个 RMS 打成条形图），不是看总时长。修法是
`ffmpeg` 砍掉慢起的头、留住本体、尾部淡出，再把画面的寿命对着 clip 的本体长度
定，两边同时开始、同时结束。

## 截一张会动的东西，要在页面里截

`page.screenshot()` 要等几帧加一次往返。一个活 0.4 秒的特效，截到的是它快消失
时的样子 —— 看起来淡，而它并不淡。

渲染器为编辑器的截图功能开着 `preserveDrawingBuffer`，所以页面里
`canvas.toDataURL()` 拿到的就是**刚画完的那一帧**。判断"好不好看"之前，先确认
自己看的是不是那一帧。

## 阈值要判**形状**，不要判拍脑袋的绝对值

「第 5 分钟身边至少 3 只」是在看到任何数据之前拍的；实测 2 只，而 2 和 3 之间
没有任何设计含义。这时候把 3 改成 2 就是搬龙门。

正确的做法是判这个系统真正承诺的**形状**：开局轻松 → 随曲线单调上升 → 后段
撞墙，再加一个后段的硬下限（那才是不能坏的地方）。

同一族：**别用极小值当判据**。「最近的一只有多远」跑三次是 1.3 / 2.7 / 1.1 ——
一个极小值由噪声主导；「半径内有几只」是聚合量，三次都给出同一个形状。

还有：**写死的阈值会随别处的改动过期**。`magnet > 3.2` 在吸取半径收到 1.4 之后
永远为假，探针报「升级看不见」而升级是好的。和「之前」比，别和常数比。

## 数 FAIL 行不是跑通了

`node probe.mjs | grep -c '^FAIL'` 对一个**崩溃**的探针也会返回 0 —— 它压根
没产出 FAIL 行。我用这个方法扫了一轮，得到一屋子绿色，其中一个探针实际上有
语法错误、根本没启动。

扫要用**退出码**：`if node $t >/tmp/o.txt 2>&1; then PASS; else FAIL; fi`。

## 一张表里的每一项都要单独验

「你跑得飞快」在 `BUFFS` 里有 id、有标签、有颜色、有脚下的圈，**代码里一个字
都没实现**，而探针只测了另外四项。没测到的那一项就是会出事的那一项。

同一族：**一个能力有两个来源时，只能有一处计算**。移动速度既能升级买、又能
道具临时给；各自直接写 `speed` 的话，道具结束时「还原」会抹掉玩家买来的那几
级——不报错，只是某一刻突然变慢了。现在只有 `applySpeed()` 一处把两者乘起来。

## 「几帧」通常是把问题问错了

「全图掉落飞向你」第一版维持 **2 帧**，实测一颗都没收到：掉落物按帧朝玩家飞，
磁吸最大 17 格/秒，两帧只够挪半格。它要的是**一段飞行时间**（1.6 秒），而且
那一下飞过来本身就是奖励的一部分。

凡是「维持 N 帧」，先问一句要维持的到底是什么——是一个开关，还是一段过程。
...
`setLevelsOff` / `setGod` / 关掉不相干的武器），而不是指望上一段收拾过自己。

`setCratesOff` 那条是**游戏系统在污染探针**，不是探针之间互相污染：一个
`wipe` 箱子在你量「400 只敌人的绘制开销」时刚好刷出来，场上瞬间清空，量到的
数字比空场还低；一个 `freeze` 会让「敌人走多快」量到 0。所以开关在游戏这边。

`setGod` 那条是后来补的：贴身伤害提到 12.8 点/秒之后，**探针在开场等待那十秒
里就被打死了**，后面所有测量都冻在结束对话框后面，读出来是「场上 0 只、平均
速度 NaN」。量别的东西的探针不该同时在打一局游戏。
