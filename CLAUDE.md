...
- **Click-to-talk dialog (NPC interaction MVP, 2026-06-28, SDK 1.0.24)** — click the cat (`catContains` world hit-test; works locked or unlocked) → a HUD dialog fades in + an HTML `<input>` overlays the canvas for typing (releases pointer lock so the field is focusable / IME-works); Enter echoes the line into the body (stub — `umicat.ai.npc` brain wires in later); Esc or click-outside closes (fade out). **The dialog is the HUD widgets with `role: "cat-dialog"`** (authored `visible: false` in `game-hud.json`: panel + "Nico" name + dynamic body bound to registry key `catDialogBody`); `GameScene.openDialog/closeDialog` show/hide them via the SDK's `getHudObjects(this, 'cat-dialog')`. **Contract for re-authoring the box in the editor:** every widget that's part of the dialog must keep `role: "cat-dialog"` + `visible: false`; the box art is a HUD `panel.backgroundAssetId` (9-slice), the typing field is the HTML input (CSS-skinned, NOT a HUD widget). Scaffold placement is rough — meant to be restyled in the HUD editor (use the 👁 hide to keep it out of the way while arranging siblings). Design: `umicat-design/features/visual-editor/16-editor-hide-lock.md` (blocks 1-3).
