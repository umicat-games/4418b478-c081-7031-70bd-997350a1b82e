...
- **Click-to-talk AI dialog — Cato (2026-06-29, SDK 1.0.27)** — click the cat (`catContains` world hit-test) → the chat HUD widgets **slide up from the bottom** (`Back.easeOut`) + an HTML `<input>` overlays the chat-input box for typing → type + Enter → **the AI (Cato) replies** (`umicat.ai.npc({ playbook:'cato', … })`, a real LLM call; player pays credits, needs sign-in) → Esc / click-outside slides it back down. **The cat is named Cato**; the bond is **guardian ↔ guarded spirit** (Pokémon-like, NOT parent/child) — defined in `public/playbooks/cato.md` (tune Cato's whole personality there, no code; an inline `role`/`style` is also passed as a preview-safe fallback if the playbook can't load).
  ...
  - **Input + message are HUD widgets now (SDK 1.0.28/1.0.29).** `chat-input-field` is a **`text-input`** widget (synced DOM `<input>`, IME-capable, SDK does the FIT+letterbox screen mapping → always aligned; emits `hud:submit`/`hud:cancel` which GameScene listens for). `chat-text` is a **`text-area`** widget (read-only DOM `<div>` overlay sized to exactly overlay the chat-message panel, 22px padding) so Cato's reply **clips to the box + scrolls** instead of overflowing. The old hand-rolled `showDialogInput`/`positionDialogInput` are gone.
  - **Cursor while chatting**: opening the dialog **releases pointer lock** (a DOM input needs key focus + a dialog wants a free cursor), but swaps the canvas CSS cursor to the game's own pixel triangle (`uploaded/triangle_mouse_icon_1.png`) so there's no jump to the host arrow; restored on close (click re-locks → CursorScene cursor returns).
  - **Pixel font**: `public/uploaded/zpix.woff2` (Zpix / 最像素, OS/2 patched to v4 for OTS + woff2 → 866KB) via `fonts.json`; chat widgets use `fontFamily: "zpix"` @ 24px. Design: `umicat-design/features/visual-editor/16-editor-hide-lock.md`.
