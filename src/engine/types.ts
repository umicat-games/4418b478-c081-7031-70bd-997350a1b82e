// VENDOR EDIT: upstream imports this from its SGF/GIB import module, which this
// game does not vendor (it only needs the engine). Widened to a string so the
// settings type still compiles; nothing here reads it.
type LegacyGameEncoding = string;
