 ...
 * fixed character count would cut "e4 is" | "where the cannon wants to be" and
 * put half a thought on the board.
 */
export function segment(text: string): Segment[] {
  ...
  const pieces = sentences
    ...
    .map((text) => {
      ...
      return marked ?? { text, at: firstPoint(text) };
    });
...
function marker(text: string): Segment | null {
  const m = /^[[［]\s*([A-Ia-i])\s?([0-9])\s*[\]］]\s*/.exec(text);
  if (!m) return null;
  const at = fromIccs(`${m[1]}${m[2]}`);
  if (!at) return null;
 ...
 */
function firstPoint(text: string): { x: number; y: number } | null {
  const re = /(^|[^A-Za-z])([A-Ia-i])\s?([0-9])(?![0-9])/g;
  let m: RegExpExecArray | null;
  while ((m = re.exec(text))) {
    const at = fromIccs(`${m[2]}${m[3]}`);
    if (at) return at;
  }
  return null;
}
...
export class Speech {
  ...
  private draw(): void {
    ...
    if (offer && at) this.playBtn.textContent = t('speech.playHere', { point: toIccs(at.x, at.y) });
