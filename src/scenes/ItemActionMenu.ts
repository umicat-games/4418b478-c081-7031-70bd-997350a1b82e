...
export function renderActionMenu(
  ...
  // highDpi: this renders in MenuScene's DEVICE-px, zoom-1 space (the modal is
  // fraction-based, not applyHudDpr'd), so every FIXED screen-px size must ×dpr or the
  // menu comes out half-size on retina. Anchor (m.x/m.y) is already device px. dpr=1 otherwise.
  const dpr = hudDpr(scene);
  const PAD = 14 * dpr, LINE = 32 * dpr, FS = Math.round(21 * dpr), SIDE = 20 * dpr, MINW = 108 * dpr;
  const S = SCALE * dpr; // 9-slice corner scale ×dpr so the border matches the larger panel
  const OFF = 36 * dpr, EDGE = 8 * dpr;
  ...
  let left = m.x + OFF;
  if (left + panelW > W - EDGE) left = m.x - OFF - panelW;
  left = Phaser.Math.Clamp(left, EDGE, Math.max(EDGE, W - panelW - EDGE));
  let top = m.y - panelH / 2;
  top = Phaser.Math.Clamp(top, EDGE, Math.max(EDGE, H - panelH - EDGE));

  // Panel behind (added first), then the option texts on top.
  if (scene.textures.exists(ATLAS)) {
    const panel = scene.add
      .nineslice(left + panelW / 2, top + panelH / 2, ATLAS, FRAME_PANEL, panelW / S, panelH / S, 10, 10, 11, 11)
      .setScale(S);
    parent.add(panel);
  } else {
    const r = scene.add.rectangle(left + panelW / 2, top + panelH / 2, panelW, panelH, 0xf2e2c4).setStrokeStyle(3 * dpr, 0x5b3a1e);
    parent.add(r);
  }
  ...
  (m.options ?? []).forEach((_o, i) => {
    ...
    bounds.push({ x: left + 10 * dpr, y: top + PAD + i * LINE, w: panelW - 20 * dpr, h: LINE, idx: i, text: texts[i], base: MENU_TEXT, hoverColor: MENU_TEXT_HOVER });
  });
  return bounds;
}
...
export function renderKeypad(
  ...
  const dpr = hudDpr(scene);
  const PAD = 18 * dpr, GAP = 8 * dpr, BW = 54 * dpr, BH = 46 * dpr, TITLE_H = 34 * dpr, DISP_H = 50 * dpr, FS = Math.round(22 * dpr);
  const S = SCALE * dpr, OFF = 36 * dpr, EDGE = 8 * dpr;
  ...
  if (scene.textures.exists(ATLAS)) {
    parent.add(scene.add.nineslice(left + panelW / 2, top + panelH / 2, ATLAS, FRAME_PANEL, panelW / S, panelH / S, 10, 10, 11, 11).setScale(S));
  } else {
  ...
  const key = (cx: number, cy: number, w: number, h: number, label: string, k: string) => {
    // 9-slice keys: size the source ÷dpr + setScale(dpr) so the border stays proportional
    // (and crisp) on the ×dpr-larger button, instead of a thin native-corner border.
    const bg: Phaser.GameObjects.GameObject = (scene.textures.exists(BTN) && scene.textures.get(BTN).has(GREY))
      ? scene.add.nineslice(cx, cy, BTN, GREY, w / dpr, h / dpr, 6, 6, 6, 6).setScale(dpr)
      : scene.add.rectangle(cx, cy, w, h, 0xd8c39a).setStrokeStyle(2 * dpr, 0x5b3a1e);
    parent.add(bg);
  ...
  if (scene.textures.exists(BTN) && scene.textures.get(BTN).has(GREY)) {
    parent.add(scene.add.nineslice((dispL + dispR) / 2, dispY, BTN, GREY, (dispR - dispL) / dpr, DISP_H / dpr, 6, 6, 6, 6).setScale(dpr));
  }
  parent.add(scene.add.text(dispR - 12 * dpr, dispY, String(m.value), { fontFamily: dialogFont(), fontSize: (FS + Math.round(6 * dpr)) + 'px', color: '#5b4327', resolution: 3 }).setOrigin(1, 0.5));
...
export function renderSlotPicker(
  ...
  // highDpi: device-px, zoom-1 space (fraction-based modal) → fixed screen-px sizes ×dpr.
  const dpr = hudDpr(scene);
  const PAD = 16 * dpr, GAP = 8 * dpr, BW = 58 * dpr, BH = 58 * dpr, TITLE_H = 32 * dpr, FS = Math.round(20 * dpr), COLS = 4;
  const S = SCALE * dpr, OFF = 36 * dpr, EDGE = 8 * dpr;
  ...
  m.slots.forEach((s, i) => {
    ...
    const bg: Phaser.GameObjects.GameObject = (scene.textures.exists(BTN) && scene.textures.get(BTN).has(GREY))
      ? scene.add.nineslice(cx, cy, BTN, GREY, BW / dpr, BH / dpr, 6, 6, 6, 6).setScale(dpr)
      : scene.add.rectangle(cx, cy, BW, BH, 0xd8c39a).setStrokeStyle(2 * dpr, 0x5b3a1e);
    parent.add(bg);
    if (s.iconKey && scene.textures.exists(s.iconKey)) {
      const icon = scene.add.image(cx, cy + 4 * dpr, s.iconKey, s.iconFrame);
      icon.setScale((BW * 0.52) / Math.max(icon.width, icon.height));
      parent.add(icon);
    }
    parent.add(scene.add.text(cx - BW / 2 + 5 * dpr, cy - BH / 2 + 3 * dpr, s.label, { fontFamily: dialogFont(), fontSize: Math.round(13 * dpr) + 'px', color: '#5b4327', resolution: 3 }).setOrigin(0, 0));
