...
//
// Three rules came out of that, and they are worth keeping in any port:
//
...
//   and the book is swept: pointers older than a few seconds are gone,
//   whatever the browser said. State that can only accumulate will.
...
export interface BoardControlsHandlers {
   ...
   *  gestures then simply do not aim — which is still what you want, because
   *  a second finger is never a move. */
...
export function attachBoardControls(
  ...
  /** How long a pointer may go unheard-of before it is assumed gone. Long
   *  enough that a slow drag is never dropped, short enough that a wedged
   *  board un-wedges itself before the player gives up on it. */
  const STALE_MS = 4000;
  ...
  const onDown = (e: PointerEvent): void => {
    ...
    // whether or not it ever said so. It is the exact answer to the bug the
    // sweep only guesses at, and it costs nothing.
  ...
  const onUp = (e: PointerEvent): void => {
    ...
    // says that finger is gone — which is the whole point of listening on the
    // window as well.
