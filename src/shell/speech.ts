 ...
 *
 * Sentences are the unit because they are the unit the assistant writes in; a
 * fixed character count would cut "H8 is" | "where the threat is" and put half
 * a thought on the board.
 *
 * The game's own `Notation` does the reading, so the shell never has an
 * opinion about what a cell is called.
 */
export function segment(text: string, notation: Notation): Segment[] {
  ...
  const pieces = sentences
    ...
    .filter(Boolean)
    .map((text) => {
      const marked = marker(text, notation);
      return marked ?? { text, at: firstPoint(text, notation) };
    });
 ...
 */
function marker(text: string, notation: Notation): Segment | null {
  const m = /^[[［]\s*([^\]］]{1,6})\s*[\]］]\s*/.exec(text);
  if (!m) return null;
  const at = notation.parse(m[1]);
  // An unparseable marker ("[the corner]") is left in the text rather than
  // silently eaten, so an assistant writing nonsense looks like an assistant
  // writing nonsense instead of like a bug in the bubble.
  if (!at) return null;
...
export function stripAnchors(text: string): string {
  return text.replace(/(^|\n)\s*[[［]\s*[A-Za-z]{1,2}\s?\d{1,2}\s*[\]］]\s*/g, '$1');
}
 ...
 * an optional space, a digit that is not part of a longer number. Without
 * that, the "e" in a word and the 2 after it become a point on the board, and
 * the bubble jumps across the river mid-sentence for no visible reason.
 */
function firstPoint(text: string, notation: Notation): { x: number; y: number } | null {
  const re = /(^|[^A-Za-z0-9])([A-Za-z]\s?\d{1,2})(?![0-9])/g;
  let m: RegExpExecArray | null;
  while ((m = re.exec(text))) {
    const at = notation.parse(m[2]);
    if (at) return at;
  }
  return null;
}
...
export interface SpeechOptions {
  ...
  /** May the student move to this point right now — with the piece they have
   *  in hand? Decides whether the bubble offers to play it. */
...
export class Speech {
  /** How this game names a cell. Swapped when the board size changes, because
  ...
  private draw(): void {
    ...
    if (offer && at) this.playBtn.textContent = t('speech.playHere', { point: this.notation.format(at) });
