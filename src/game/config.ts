 ...
 * 持枪视角：枪在屏幕上的位置与朝向。
 *
 * **这些数字原本写了两遍** —— 构造函数里一份，每帧的摇摆/后坐力里又一份，
 * 而每帧那份从第一帧起就把前者盖掉了。所以改构造函数那行是完全没反应的，
 * 这正是本项目记过的坑（村庄的出生点在场景和代码里各写一次，代码赢，
 * 于是改场景毫无作用）。现在只有这一处。
 *
 * 每帧的摇摆、呼吸起伏和后坐力都是**加在**这个基准姿态上的，不是替换它。
 */
export const GUN_POSE = {
  /** 左右。正数在右手边，负数搬到左手。 */
  x: 0.25,
  /** 上下。负数在视线以下。 */
  y: -0.24,
  /** 前后。相机朝 -z 看，所以越负离得越远、枪越小。 */
  z: -0.5,
  /** 枪口左右摆（弧度）。正数枪口向右偏。 */
  yaw: 0,
  /** 枪口上下抬（弧度）。正数枪口抬高。 */
