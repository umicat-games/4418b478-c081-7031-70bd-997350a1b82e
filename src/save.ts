...
//
// A game is saved as its MOVES rather than as a board: smaller, replayable,
// and it means a save can never contain a position the rules cannot reach —
// `Gomoku.restore` replays through the same referee the player moves through.
//
...
import type { ChatMessage, Profile } from './shell/coach';
import type { OthelloSnapshot } from './game/rules';
...
export interface Saved {
  ...
  game: OthelloSnapshot | null;
  /** True when this player has been here before — what "Continue" needs. */
  returning: boolean;
}

export async function load(umicat: ThreeUmicat, defaults: Profile): Promise<Saved> {
  const [profile, messages, game] = await Promise.all([
    umicat.saves.get<Profile>(KEY.profile),
    umicat.saves.get<ChatMessage[]>(KEY.chat),
    umicat.saves.get<OthelloSnapshot>(KEY.game),
  ]);
  return {
    profile: { ...defaults, ...(profile ?? {}), game: { ...defaults.game, ...(profile?.game ?? {}) } },
    messages: messages ?? [],
    game: game ?? null,
...
export class Autosave {
  ...
  async flush(): Promise<void> {
    ...
    await Promise.all(writes).catch((err) => console.warn('[gomoku] save failed', err));
  }
}
