...
GID=2a991d0a-30ae-47a8-b04c-a726e5eb2fbe
DIST="$REPO/dist"
BUCKET=unboxy-dev
...
# The engine: 360KB of wasm that never changes, and a content type browsers
# are strict about. Without `application/wasm` the streaming compile path
# refuses it and emscripten falls back to a slower one with a console warning
# that looks exactly like a broken build.
echo "==> the engine, as wasm"
if [ -d "$DIST/stockfish" ]; then
  aws s3 cp "$DIST/stockfish/stockfish.wasm" "s3://$BUCKET/previews/$GID/stockfish/stockfish.wasm" \
